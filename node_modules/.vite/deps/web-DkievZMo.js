...
var FirebaseAuthenticationWeb = class FirebaseAuthenticationWeb extends WebPlugin {
	...
	async fetchSignInMethodsForEmail(options) {
		const auth = getAuth();
		return { signInMethods: await fetchSignInMethodsForEmail(auth, options.email) };
	}
	...
	async isSignInWithEmailLink(options) {
		const auth = getAuth();
		return { isSignInWithEmailLink: isSignInWithEmailLink(auth, options.emailLink) };
	}
	...
	async setPersistence(options) {
		...
		switch (options.persistence) {
			...
			case Persistence.IndexedDbLocal:
				...
				break;
			case Persistence.InMemory: await setPersistence(auth, inMemoryPersistence);
		}
	}
